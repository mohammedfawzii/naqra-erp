...
class ModuleSeederService
{
    public static function make(string $module, string $model)
    {
        ...
        $modelSeederFolder = base_path("Modules/{$module}/database/seeders/{$model}");
...
";
    ...
    {
        $moduleSeederPath = base_path("Modules/{$module}/database/seeders/{$module}DatabaseSeeder.php");

        if (!File::exists($moduleSeederPath)) {
            // ✅ namespace هنا برضو يكون Database\Seeders
            $stub = "<?php
