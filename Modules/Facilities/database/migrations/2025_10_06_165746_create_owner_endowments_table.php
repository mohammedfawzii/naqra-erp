<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration {
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('owner_endowments', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('owner_id');

            $table->string('endowment_name')->nullable();
            $table->string('endowment_national_number')->nullable();
            $table->date('approval_date')->nullable();
            $table->string('authorized_person')->nullable();
            $table->string('authorized_mobile')->nullable();
            $table->string('authorized_email')->nullable();
            $table->string('address')->nullable();
            $table->string('partnership_type')->nullable();
            $table->integer('partnership_percentage')->nullable();
            $table->text('note')->nullable();
            $table->timestamps();
            $table->foreign('owner_id')->references('id')->on('owners')->onDelete('cascade');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('owner_endowments');
    }
};
