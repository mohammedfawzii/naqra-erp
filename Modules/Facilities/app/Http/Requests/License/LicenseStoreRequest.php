<?php

namespace Modules\Facilities\Http\Requests\License;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Http\Exceptions\HttpResponseException;
use Illuminate\Contracts\Validation\Validator;

class LicenseStoreRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    public function rules(): array
    {
        return [
            'ministry_name_id' => 'required|integer|exists:ministries,id',
            'license_address' => 'required|string|max:255',
            'license_number' => 'required|string|max:255',
            'license_type_id' => 'required|string|max:255|exists:license_types,id',
            'license_value' => 'required|string',
            'branch_area' => 'required|string',
            'license_start_date' => 'required|date',
            'license_end_date' => 'required|date',
            'branch_id' => 'nullable|integer|exists:branches,id',
            'facility_attachments_id' => 'required|string|max:255|exists:facility_attachments,reference_number',

        ];
    }

    public function failedValidation(Validator $validator)
    {
        throw new HttpResponseException(response()->json([
            'success' => false,
            'message' => 'Validation errors',
            'error' => $validator->errors()
        ], 422));
    }
}
